...
Another example would be trying to open a VPN connection from within a very restrictive network. In some cases port 587, or even UDP traffic on any port, may be restricted by network policy. Because TCP traffic on port 443 is used for normal TLS (https) web browsing, it is very unlikely to be blocked.
...
Because the server container always exposes port 587, regardless of the
specified protocol, adjust the mapping appropriately:

    docker run -v $OVPN_DATA:/etc/openvpn -d -p 443:587/tcp --cap-add=NET_ADMIN kylemanna/openvpn

## Running a Second Fallback TCP Container
Instead of choosing between UDP and TCP, you can use both. A single instance of OpenVPN can only listen for a single protocol on a single port, but this image makes it easy to run two instances simultaneously. After building, configuring, and starting a standard container listening for UDP traffic on 587, you can start a second container listening for tcp traffic on port 443:

    docker run -v $OVPN_DATA:/etc/openvpn --rm -p 443:587/tcp --cap-add=NET_ADMIN kylemanna/openvpn ovpn_run --proto tcp
